...
def generate_mask_from_png(png_path, threshold, output_nii_path, target_shape=(256, 256, 1)):
    """
    从 PNG 图像生成掩码并保存为 NIfTI 格式
    :param png_path: 输入 PNG 图像的路径
    :param threshold: 用于生成掩码的像素阈值
    :param output_nii_path: 输出 NIfTI 掩码文件的路径
    :param target_shape: 目标图像形状，默认为 (256, 256, 1)
    """
    try:
        # 打开 PNG 图像
        ...
        # 调整图像大小到二维目标形状
        target_2d_shape = target_shape[:2]
        img = img.resize(target_2d_shape, Image.BICUBIC)
        ...
        if len(target_shape) == 3:
            if target_shape[2] > 1:
                mask = np.repeat(mask[:, :, np.newaxis], target_shape[2], axis=2)
...
if __name__ == "__main__":
    ...
    # precheck the image mask shape if existed
    precheck = False
    if precheck:
        mask_nii = nib.load("./example/mask.nii")
